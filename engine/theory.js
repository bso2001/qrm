...
function findIntervals( song, part )
{
        if ( part.passingNotes && part.passingNotes.length > 0 ) {
                ...
                for ( const pn of part.passingNotes ) {
                        const nn = _degreeToSemitone( pn, part.keyRoot || song.keyRoot )
                        pni.push( nn )
                }
        ...
        throw new Error( "Unsupported mode: " + key.mode )
}
